...
func TestRunTasks(t *testing.T) {
	...
	t.Run("run tasks with custom type struct", func(t *testing.T) {
		...
		result, err := RunTasks[ResultType](p, ctx, tasks...)
	...
	t.Run("run tasks with provider func", func(t *testing.T) {
		result, err := RunTasksWithSupplierFunc[int64](p, ctx, func(outch chan<- Task) {
			for i := 1; i <= 3; i++ {
				v := i
				outch <- func() (any, error) {
					return int64(v), nil
				}
			}
		})
	...
	t.Run("run task group with supplier", func(t *testing.T) {
		...
		result, err := RunTaskGroupWithSupplierFunc[int64](tg, func(outch chan<- Task) {
			for i := 1; i <= 3; i++ {
				v := i
				outch <- func() (any, error) {
					return int64(v), nil
